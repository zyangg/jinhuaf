  ...
  <div class="container" style="margin-top: 10px">
      ...
      <div class="col-12 col-md-8">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div>如果你喜欢这篇帖子可以给点个小爱心
              <span class="iconfont icon-xihuan" @click="likeReply()"></span></div>
          </div>
        </div>
        <div class="textarea">
    ...
    <div class="row justify-content-center">
      <div class="col-12 col-md-8" v-for="(item,index) in replyData" :key="index">
        <div class="card">
          <div class="card-body">
            <span><el-button type="text" @click="getName($event, item._id, index)">
              {{item.name}}</el-button>说:{{item.desc}}</span>
              <span v-for="(item1,index1) in reply1Data[index]" :key="index1">
                <br>
                <el-button type="text" @click="getName($event, item._id, index)">
              {{item1.name2}}
              </el-button>对
              <el-button type="text" @click="getName($event, item._id, index)">{{item1.name1}}
                </el-button> 说:{{item1.desc}}</span>
          </div>
      ...
      this.$axios
        ...
        .then(async (res) => {
          ...
          for (var i = 0; i < this.replyData.length; i++) {
            await this.$axios.post('/getReply1', {
